...
const AdminDashboard = () => {
  ...
  const renderOperacao = () => {
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="max-w-[1200px] mx-auto py-12 px-8">
          <div className="mb-12 flex justify-between items-end">
            ...
            <button 
              ...
              className="bg-white text-black px-6 py-3 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-white/90 transition-colors cursor-pointer"
            >
    ...
    if (activeMenu === "Projetos") {
      const clientsList = ["Todos", ...Array.from(new Set(pipeline.map((p: any) => p.client)))];
      
      // Filter by tab and remove duplicates to avoid the repetitive entries issue
      const uniqueProjects = pipeline.reduce((acc: any, curr: any) => {
        if (!acc.find((p: any) => p.client === curr.client)) {
          acc.push(curr);
        }
        return acc;
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="max-w-[1200px] mx-auto py-12 px-8">
          <div className="mb-12 flex flex-col md:flex-row md:justify-between md:items-end gap-6">
            ...
            <button 
              ...
              className="bg-white text-black px-6 py-3 rounded-xl text-sm font-bold flex items-center gap-2 hover:bg-white/90 transition-colors cursor-pointer w-fit"
            >
              <Plus className="w-4 h-4" /> Novo Projeto
            </button>
          </div>

          <div className="flex gap-2 p-1.5 bg-white/[0.03] border border-white/10 rounded-2xl mb-8 overflow-x-auto w-fit max-w-full">
            {clientsList.map((clientName: any) => (
              <button 
                key={clientName}
                onClick={() => setProjectClientTab(clientName)}
                className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-all whitespace-nowrap ${
                  projectClientTab === clientName ? "bg-white text-black shadow-lg" : "text-white/50 hover:text-white"
                }`}
              >
                {clientName}
              </button>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {uniqueProjects.map((p: any, i: number) => {
              const progressNum = parseInt(p.progress) || 0;
              return (
                <div key={i} className="bg-[#050505] border border-white/10 hover:border-white/20 rounded-3xl p-6 transition-all group flex flex-col h-full relative overflow-hidden">
                  <div className="absolute top-0 right-0 w-32 h-32 bg-white/[0.02] rounded-bl-full -mr-8 -mt-8 pointer-events-none transition-colors group-hover:bg-white/[0.04]"></div>
                  
                  <div className="flex justify-between items-start mb-6">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-xl bg-white/5 flex items-center justify-center text-sm text-white/70 border border-white/10 font-bold shadow-inner">
                        {p.client.charAt(0)}
                      </div>
                      <div>
                        <h3 className="font-semibold text-white/90 text-base">{p.client}</h3>
                        <p className="text-[10px] font-mono text-white/40 uppercase tracking-wider mt-0.5">Website Institucional</p>
                      </div>
                    </div>
                  ...
                  <div className="mb-6 flex-grow">
                    <div className="flex justify-between text-sm mb-2">
                      ...
                      <span className="text-white/90 font-mono">{p.progress}</span>
                    </div>
                    <div className="w-full h-1.5 bg-white/5 rounded-full overflow-hidden">
                      <div className="h-full bg-[#009EE3] rounded-full" style={{ width: `${progressNum}%` }}></div>
                    </div>
                  </div>

                  <div className="flex items-center justify-between pt-4 border-t border-white/5 mt-auto">
                    <div className="flex flex-col">
                      <span className="text-[10px] font-mono text-white/40 uppercase tracking-wider mb-1">Status Financeiro</span>
                      <span className={`inline-flex items-center gap-1.5 text-xs font-medium ${p.payment === 'Pago' ? 'text-emerald-400' : p.payment === 'Pendente' ? 'text-amber-400' : 'text-red-400'}`}>
                        <div className={`w-1.5 h-1.5 rounded-full ${p.payment === 'Pago' ? 'bg-emerald-400' : p.payment === 'Pendente' ? 'bg-amber-400' : 'bg-red-400'}`}></div>
                        {p.payment}
                      </span>
                    </div>
                    <button 
                      onClick={() => setSelectedClient(p)}
                      className="bg-white/5 hover:bg-white/10 text-white px-4 py-2 rounded-xl text-xs font-medium flex items-center gap-2 transition-colors border border-white/5"
                    >
            ...
            {uniqueProjects.length === 0 && (
              <div className="col-span-full py-20 flex flex-col items-center justify-center text-center border border-white/5 border-dashed rounded-3xl bg-white/[0.01]">
                <Briefcase className="w-8 h-8 text-white/20 mb-4" />
                <p className="text-white/50 font-mono text-xs uppercase tracking-widest">Nenhum projeto encontrado</p>
              </div>
            )}
          </div>
        </motion.div>
          ...
          {clientTab === "Materiais" && (
            ...
            >
               <div className="flex justify-between items-center mb-8">
                 <h3 className="text-[13px] font-mono text-white/40 uppercase tracking-wider">Entregáveis do Projeto</h3>
               ...
               <div className="space-y-6">
                 {[
                   { title: "Identidade Visual", status: "Aprovado", items: [{name: "Logo em Vetor", file: "logo_final.ai", size: "2.4 MB"}, {name: "Manual da Marca", file: "brandbook.pdf", size: "15 MB"}, {name: "Tipografia", file: "fonts.zip", size: "8 MB"}] },
                   { title: "Copywriting", status: "Pendente", items: [{name: "Textos da Home", file: null}, {name: "Sobre nós", file: null}, {name: "Serviços", file: null}] },
                   { title: "Mídia & Fotografia", status: "Em revisão", items: [{name: "Fotos do Escritório", file: "fotos_escritorio.zip", size: "120 MB"}, {name: "Retratos da Equipe", file: null}] }
                 ].map((section, idx) => (
                   <div key={idx} className="bg-white/[0.02] border border-white/10 rounded-2xl overflow-hidden">
                      <div className="flex justify-between items-center p-6 border-b border-white/5 bg-white/[0.01]">
                        ...
                        <span className={`inline-flex items-center px-3 py-1.5 rounded-xl text-[10px] uppercase tracking-wider font-mono border ${getStatusColor(section.status)}`}>
                          {section.status}
                        </span>
                      </div>
                      ...
                      <div className="divide-y divide-white/5">
                        {section.items.map((item, i) => (
                          <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between p-6 hover:bg-white/[0.02] transition-colors group gap-4">
                            <div className="flex items-center gap-4">
                                <div className={`w-10 h-10 shrink-0 rounded-xl flex items-center justify-center border ${item.file ? 'bg-green-500/10 border-green-500/20 text-green-500' : 'bg-white/5 border-white/10 text-white/30'}`}>
                                  {item.file ? <CheckCircle2 className="w-5 h-5" /> : <File className="w-5 h-5" />}
                                </div>
                                <div>
                                  <span className="text-sm text-white/90 font-medium block mb-1">{item.name}</span>
                                  {item.file ? (
                                    <span className="text-[11px] font-mono text-white/40">{item.file} • {item.size}</span>
                                  ) : (
                                    <span className="text-[11px] font-mono text-orange-500/70 uppercase tracking-wider flex items-center gap-1.5">
                            ...
                            <div className="flex items-center gap-3 self-end sm:self-auto">
                              {item.file ? (
                                <>
                                  <button className="bg-white/5 hover:bg-white/10 text-white/70 hover:text-white px-4 py-2 rounded-xl text-xs font-medium transition-colors border border-white/5 flex items-center gap-2">
                                    <Download className="w-4 h-4" /> Baixar
                                  </button>
                                  <button className="bg-white/5 hover:bg-red-500/10 text-white/70 hover:text-red-500 px-4 py-2 rounded-xl text-xs font-medium transition-colors border border-white/5 flex items-center gap-2">
                                    <Trash2 className="w-4 h-4" />
                                  </button>
                                </>
                              ) : (
                                <>
                                  <button className="bg-orange-500/10 hover:bg-orange-500/20 text-orange-500 px-4 py-2 rounded-xl text-xs font-medium transition-colors border border-orange-500/20 flex items-center gap-2">
                                    <Bell className="w-4 h-4" /> Alerta SMS
                                  </button>
                                  <button className="bg-white hover:bg-white/90 text-black px-4 py-2 rounded-xl text-xs font-bold transition-colors flex items-center gap-2">
                                    <UploadCloud className="w-4 h-4" /> Anexar
                                  </button>
                                </>
                              )}
                            </div>
    ...
    <AnimatePresence>
      {isNewClientModalOpen && (
        ...
        >
          ...
          >
            <button onClick={() => setIsNewClientModalOpen(false)} className="absolute top-6 right-6 text-white/40 hover:text-white">
              <X className="w-5 h-5" />
            </button>
            <h2 className="text-2xl font-bold text-white mb-2">Novo Cliente</h2>
            <p className="text-white/50 text-sm mb-6">Adicione um novo cliente à sua carteira.</p>
            
            <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setIsNewClientModalOpen(false); }}>
              <div>
                <label className="block text-[11px] font-mono text-white/40 mb-2 uppercase tracking-wider">Nome da Empresa</label>
                <input required type="text" className="w-full bg-[#050505] border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-[#009EE3] transition-colors" placeholder="Ex: Casa Lellit" />
              </div>
              <div>
                <label className="block text-[11px] font-mono text-white/40 mb-2 uppercase tracking-wider">E-mail Principal</label>
                <input required type="email" className="w-full bg-[#050505] border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-[#009EE3] transition-colors" placeholder="Ex: contato@empresa.com" />
              </div>
              <div>
                <label className="block text-[11px] font-mono text-white/40 mb-2 uppercase tracking-wider">CNPJ / CPF</label>
                <input required type="text" className="w-full bg-[#050505] border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-[#009EE3] font-mono transition-colors" placeholder="00.000.000/0001-00" />
              </div>
              <button type="submit" className="w-full bg-white text-black font-bold py-3 rounded-xl mt-4 hover:bg-white/90 transition-colors">
                Cadastrar Cliente
              </button>
            </form>
          </motion.div>
      ...
      {isNewProjectModalOpen && (
        ...
        >
          <motion.div 
            ...
            className="bg-[#0a0a0a] border border-white/10 rounded-3xl p-8 max-w-md w-full relative"
          >
            <button onClick={() => setIsNewProjectModalOpen(false)} className="absolute top-6 right-6 text-white/40 hover:text-white">
              <X className="w-5 h-5" />
            </button>
            <h2 className="text-2xl font-bold text-white mb-2">Novo Projeto</h2>
            <p className="text-white/50 text-sm mb-6">Inicie um novo projeto para um cliente.</p>
            
            <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setIsNewProjectModalOpen(false); }}>
              <div>
                <label className="block text-[11px] font-mono text-white/40 mb-2 uppercase tracking-wider">Cliente</label>
                <select required className="w-full bg-[#050505] border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-[#009EE3] transition-colors appearance-none">
                  <option value="" disabled selected>Selecione um cliente</option>
                  {dashboardData?.pipeline?.reduce((acc: any, curr: any) => acc.find((c: any) => c.client === curr.client) ? acc : [...acc, curr], []).map((p: any) => (
                    <option key={p.client} value={p.client}>{p.client}</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-[11px] font-mono text-white/40 mb-2 uppercase tracking-wider">Nome do Projeto</label>
                <input required type="text" className="w-full bg-[#050505] border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-[#009EE3] transition-colors" placeholder="Ex: Website Institucional" />
              </div>
              <div>
                <label className="block text-[11px] font-mono text-white/40 mb-2 uppercase tracking-wider">Valor Global (R$)</label>
                <input required type="number" step="0.01" className="w-full bg-[#050505] border border-white/10 rounded-xl px-4 py-3 text-white font-mono outline-none focus:border-[#009EE3] transition-colors" placeholder="0.00" />
              </div>
              <button type="submit" className="w-full bg-white text-black font-bold py-3 rounded-xl mt-4 hover:bg-white/90 transition-colors">
                Criar Projeto
              </button>
