    ...
    <div className="py-10 px-8 xl:px-12 w-full max-w-7xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        <div className="space-y-1">
          ...
          <p className="text-zinc-500 text-sm font-medium">Coordene o pipeline comercial e o funil de aquisição.</p>
        </div>
        <button onClick={openCreate}
          ...
          <Plus className="w-4 h-4 stroke-[3]" /> Registrar Novo Lead
        </button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          { label: "Buffer de Aquisição", val: leads.length, icon: Target, color: "text-white", glow: "white" },
          { label: "Pipeline Ativo", val: activeCount, icon: UserPlus, color: "text-blue-400", glow: "blue" },
          { label: "Conversão (Ganho)", val: wonCount, icon: Star, color: "text-emerald-400", glow: "emerald" },
        ].map((k, i) => (
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <div className="py-32 text-center">
              ...
              <p className="text-zinc-500 text-sm font-medium">Nenhum lead encontrado no pipeline.</p>
            </div>
          ) : (
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/5 text-[10px] text-zinc-500 uppercase font-bold tracking-widest">
                  {["Indivíduo / Entidade", "Matriz de Contato", "Origem", "Estágio do Funil", "Operações"].map(h => (
      ...
      <Modal
        ...
        title={modal === "create" ? "Inicializar Prospecção" : "Inteligência de Prospecção"}
        description="Gerencie dados de contato comercial e posicionamento no funil."
        maxWidth="md"
        footer={
          <div className="flex items-center justify-between w-full pt-6 border-t border-white/5">
            {modal === "edit" ? (
               <button type="button" onClick={() => remove(form.id)} className="cursor-pointer text-[10px] text-red-500/50 hover:text-red-500 font-bold uppercase tracking-widest transition-all">ENCERRAR PROSPECÇÃO</button>
            ) : <div/>}
            <div className="flex gap-4">
              ...
              <button onClick={save} disabled={saving} className="cursor-pointer px-8 py-2.5 bg-white text-black rounded-full text-xs font-extrabold uppercase tracking-widest hover:bg-zinc-200 transition-all shadow-[0_8px_20px_rgba(255,255,255,0.1)] active:scale-95 disabled:opacity-50">
                {saving ? "Sincronizando..." : modal === "create" ? "Registrar Prospecto" : "Confirmar Alterações"}
              </button>
            </div>
          </div>
        }
        ...
        <div className="space-y-8 py-4">
          <div className="grid grid-cols-1 gap-6">
            <div className="space-y-2">
              <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em]">Identidade do Contato / Entidade</label>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em]">Matriz de Comunicação (E-mail)</label>
              ...
              <div className="space-y-2">
                <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em]">Linha Direta (WhatsApp)</label>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em]">Posicionamento no Funil</label>
                <select value={form.status ?? "new"} onChange={e => setForm((f: any) => ({ ...f, status: e.target.value }))}
                  ...
                  <option value="new">Lead Inicial</option>
                  <option value="qualified">Ativo Qualificado</option>
                  <option value="meeting">Briefing Agendado</option>
                  <option value="proposal">Proposta Enviada</option>
                  <option value="won">Conversão Bem-sucedida</option>
                  <option value="lost">Oportunidade Perdida</option>
                </select>
              </div>
              <div className="space-y-2">
                <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em]">Origem da Aquisição</label>
                <select value={form.source ?? "site"} onChange={e => setForm((f: any) => ({ ...f, source: e.target.value }))}
                  className="w-full bg-white/[0.03] border border-white/5 rounded-2xl px-5 py-4 text-white text-sm outline-none focus:border-white/10 focus:bg-white/[0.05] transition-all appearance-none cursor-pointer">
                  <option value="site">Orgânico / Inbound</option>
                  <option value="ads">Tráfego Pago (Ads)</option>
                  <option value="referral">Indicação / Network</option>
            ...
            <div className="space-y-2">
              <label className="block text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em]">Inteligência de Contato (Notas)</label>
              <textarea value={form.notes ?? ""} onChange={e => setForm((f: any) => ({ ...f, notes: e.target.value }))}
                rows={4} className="w-full bg-white/[0.03] border border-white/5 rounded-2xl px-5 py-4 text-zinc-300 text-sm outline-none focus:border-white/10 focus:bg-white/[0.05] transition-all resize-none" placeholder="Observações estratégicas, sinais de intenção, contexto histórico..." />
            </div>
          </div>
        </div>
      </Modal>
