...
const SOLUTIONS = [
  {
    ...
    desc: 'Centralize processos, equipes e informações em uma única plataforma.',
  ...
  },
  {
    ...
    desc: 'Elimine tarefas repetitivas e reduza gargalos operacionais.',
  ...
  },
  {
    ...
    desc: 'Sistemas desenvolvidos para o fluxo específico da sua empresa.',
  ...
  },
  {
    ...
    desc: 'Produtos digitais rápidos, performáticos e preparados para escala.',
...
export function SolutionsSection() {
  return (
    <section className="relative z-20 py-16 md:py-24 px-4 md:px-12 bg-[#050505] overflow-hidden">
      <div className="w-full max-w-[1400px] mx-auto relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10 mb-6">
            ...
            <span className="text-[10px] font-mono font-bold text-white tracking-[0.2em] uppercase">SOLUÇÕES</span>
          </div>
          <h2 className="text-4xl md:text-5xl lg:text-6xl font-bold text-white tracking-tighter mb-6 max-w-3xl">
            O que posso construir para sua empresa.
          </h2>
          <p className="text-white/50 text-lg md:text-xl max-w-2xl font-light">
            Desenvolvo sistemas, automações e plataformas privadas para transformar operações manuais em processos organizados, escaláveis e previsíveis.
          </p>
        </motion.div>
            ...
            {SOLUTIONS.map((solution, index) => (
              ...
              >
                ...
                <div className="w-[120px] md:w-[140px] h-8 md:h-10 bg-neutral-900/80 backdrop-blur-md border-t border-l border-r border-white/10 rounded-t-xl flex items-center px-4 md:px-5 relative z-10 transition-colors group-hover:border-white/20 group-hover:bg-neutral-800/80">
                  <span className="text-[9px] md:text-[10px] font-mono font-bold text-white/40 group-hover:text-white/80 transition-colors tracking-widest uppercase truncate">
                    0{index + 1}
                  </span>
                ...
                {/* Folder Body */}
                <div className="bg-neutral-900/80 backdrop-blur-md border border-white/10 rounded-b-2xl rounded-tr-2xl p-6 relative z-10 group-hover:border-white/20 transition-colors flex-1 flex flex-col shadow-2xl group-hover:bg-neutral-800/80">
                  
                  {/* Subtle top glow effect inside card on hover */}
                  <div className="absolute top-0 left-1/2 -translate-x-1/2 w-1/2 h-[2px] bg-gradient-to-r from-transparent via-white/30 to-transparent opacity-0 group-hover:opacity-100 blur-[2px] transition-opacity duration-500" />
                  
                  <div className="w-12 h-12 rounded-xl bg-white/[0.02] border border-white/[0.05] flex items-center justify-center mb-6 group-hover:bg-white/5 group-hover:border-white/10 transition-colors duration-300">
                    <solution.icon className="w-6 h-6 text-white/40 group-hover:text-white transition-colors duration-300" />
                  </div>
                  ...
                  <ul className="flex flex-col gap-3 mb-8 flex-1">
                    {solution.features.map((feature, i) => (
                      <li key={i} className="flex items-center gap-3 text-sm text-white/70">
                        <div className="flex-shrink-0 w-4 h-4 rounded-full bg-white/10 flex items-center justify-center">
                  ...
                  <div className="mt-auto pt-6 border-t border-white/5 group-hover:border-white/10 transition-colors">
                    <a 
                      ...
                      className="inline-flex items-center gap-2 text-xs font-bold font-mono tracking-wider uppercase text-white/50 group-hover:text-white transition-colors"
                    >
