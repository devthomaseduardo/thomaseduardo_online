...
const SMOOTH_TRANSITION = { type: "spring", stiffness: 40, damping: 20 } as const;
const FADE_UP = {
        ...
        >
          <div className="flex flex-col md:flex-row md:items-center gap-4 mb-8">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/5 border border-white/10">
              <Activity className="w-3 h-3 text-[#A1A1AA] animate-pulse" />
              <span className="text-[10px] font-mono font-bold text-[#A1A1AA] tracking-[0.2em] uppercase">{t.solutions.eyebrow}</span>
            </div>
            ...
            {SOLUTIONS.map((solution, index) => (
              ...
              >
                ...
                <div className={`w-[120px] md:w-[140px] h-8 md:h-10 border-t border-l border-r rounded-t-xl flex items-center px-4 md:px-5 relative z-10 transition-all duration-500 ${
                  expandedIndex === index 
                  ? 'bg-[#0A0A0A] border-[#18181B] backdrop-blur-xl' 
                  : 'bg-neutral-900/80 border-white/10 backdrop-blur-md group-hover:border-white/20'
                }`}>
                  <span className={`text-[9px] md:text-[10px] font-mono font-bold transition-colors tracking-widest uppercase truncate ${
                  expandedIndex === index ? 'text-white' : 'text-white/40 group-hover:text-white'
                  }`}>
                    Module_0{index + 1}
                  </span>
                  <div className={`absolute -bottom-[1px] left-0 w-full h-[2px] z-20 transition-colors ${
                  expandedIndex === index ? 'bg-[#0A0A0A]' : 'bg-neutral-900/80'
                  }`} />
                </div>
                ...
                <div className={`backdrop-blur-md border rounded-b-2xl rounded-tr-2xl p-6 relative z-10 transition-all duration-500 flex-1 flex flex-col shadow-2xl ${
                  expandedIndex === index 
                  ? 'bg-[#0A0A0A] border-[#18181B]' 
                  : 'bg-neutral-900/80 border-white/10 group-hover:border-white/20 group-hover:bg-neutral-800/80'
                }`}>
                  <div className="flex items-start justify-between mb-6">
                    <div className={`w-12 h-12 rounded-xl border flex items-center justify-center transition-all duration-500 ${
                      expandedIndex === index 
                      ? 'bg-white/10 border-white/20' 
                      : 'bg-white/[0.02] border-white/[0.05] group-hover:bg-white/5 group-hover:border-white/10'
                    }`}>
                      <solution.icon className={`w-6 h-6 transition-colors duration-300 ${
                      expandedIndex === index ? 'text-white' : 'text-white/40 group-hover:text-white/80'
                      }`} />
                    </div>
                    <ChevronDown 
                    className={`w-5 h-5 text-white/20 group-hover:text-white/50 transition-all duration-500 ${expandedIndex === index ? 'rotate-180 text-white' : ''}`}
                    />
                  </div>
                  ...
                  <AnimatePresence initial={false}>
                    {expandedIndex === index ? (
                      ...
                      >
                        ...
                        <div 
                      className="mt-auto pt-6 border-t border-white/5 group-hover:border-white/10 transition-colors"
                        ...
                        className="inline-flex items-center gap-2 text-[10px] font-bold font-mono tracking-widest uppercase text-white/60 hover:text-white transition-colors"
                          >
